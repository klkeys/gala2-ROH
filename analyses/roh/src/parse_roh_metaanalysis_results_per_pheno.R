...
option_list = list(
    ...
    make_option(
        ...
        type    = "character",
        default = NULL,
        help    = "Name of the phenotype to analyze, e.g. 'Asthma_Status'",
        metavar = "character"
    ),
    make_option(
...
#markers = fread(marker.file, header = FALSE)

# marker.file lacks a header, so we add it here
#colnames(markers) = c("Probe")

# split the Probe (MarkerName) column into chr and bp
# save the Probe column (remove = FALSE) since that is also the SNP name
# purge unusually tiny p-values since in ROH association those are probably artifactual
x = results %>% 
    tidyr::separate(
        MarkerName,
        into = c("CHR", "BP"),
...
merged.results.filename = file.path(output.dir, paste(pheno.name, "ROH.metaanalysis", "txt", "gz", sep = "."))
fwrite(x, file = merged.results.filename, sep = "\t", quote = FALSE)

# create manhattan plot
manhattan.plot.filepath = file.path(output.dir, paste(pheno.name, pop.code, "manhattan", plot.filetype, sep = "."))
...
# make QQ plot
qq.plot.filepath = file.path(output.dir, paste(pheno.name, pop.code, "qq", plot.filetype, sep = "."))
g2 = CreateQQPlot(x,
    title = paste("Meta-analysis of", phenotype.names[[pheno.name]], sep = " "),
...
rdata.filepath = file.path(output.dir, paste(pheno.name, pop.code, "plots", "Rdata", sep = "."))
